...
export const createSandboxChatSession = (weights: Record<string, number>, rigidity: number, isWeightingEnabled: boolean): Chat => {
    ...
    try {
        ...
        if (outputType === 'batch' || outputType === 'story' || outputType === 'video') {
            ...
            try {
                ...
                return response.text; // Return raw JSON for story/video
            } catch (e) {
                return `Error parsing JSON response: ${response.text}`;
            }
        }
